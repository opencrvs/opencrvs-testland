...
test(
  'Scope changes after office change - user loses access when the office changes',
  async ({ browser }) => {
    ...
    const page = await browser.newPage()
      ...
      await test.step(
        'Create a new registrar user and set up initial record and drafts',
        async () => {
      ...
      await test.step(
        'Local administrator moves the user to Isamba District Office',
        async () => {
          await logout(page)
      ...
      await test.step(
        'User can no longer find the record or drafts after the office change',
        async () => {
          await logout(page)
